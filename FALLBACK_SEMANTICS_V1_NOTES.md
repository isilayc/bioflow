...
2. **Alternative workflow strategy** — when changing the tool would actually change the scientific method, BioFlow suggests another complete route for the same dataset context instead of silently swapping methods.
3. **No equivalent fallback / remediation** — when the catalog has no equivalent substitute, BioFlow says so and explains how to repair the current route.
